...
Choicekit keeps state history without cloning the entire state object on every change. It does this by storing:
...
export const engine = await ChoicekitEngine.init({
...
A **snapshot** is a partial object containing only the properties you changed since the previous snapshot. Choicekit stores snapshots in an array (the **state history**) and uses an internal `index` to decide which point in that history represents “now”.
...
- Choicekit records your edits into the current snapshot
