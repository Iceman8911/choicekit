...
## Current official example plugins
...
These two examples demonstrate the key plugin patterns:

1. Stateful API mounted at `engine.$`
2. Event emitter API for feature-specific events
3. Persistent plugin state (`withSave: false`) for out-of-save progression/preferences
